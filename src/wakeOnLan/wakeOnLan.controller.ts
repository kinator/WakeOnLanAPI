...
function execute(command: string, callback: (arg0: string) => void){
  exec(command, function(error, stdout, stderr){ callback(stdout); });
};

export const getStatus = async (_req: Request, res: Response) => {
  try {
    const IP: string = process.env.SERVER_IP ?? "nope"
...
export const shutdown = async (req: Request, res: Response) => {
  try {
    const OS: String = req.body.currentOS
...
export const powerOn = async (_req: Request, res: Response) => {
  try {
